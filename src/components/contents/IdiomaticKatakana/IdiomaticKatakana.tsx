...
const query = graphql`
  query IdiomaticKatakana {
    idiomaticKatakana: allSdsAirtable(filter: { table: { eq: "用字一覧：カタカナ語" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticKatakana: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticKatakanaQuery>(query)

  const idiomaticKatakana = data.idiomaticKatakana.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticKatakana[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticKatakana.map(({ name, description, discussion, source, recordId }, index) => {
