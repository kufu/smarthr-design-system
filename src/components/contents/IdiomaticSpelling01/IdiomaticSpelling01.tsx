...
const query = graphql`
  query IdiomaticSpelling01 {
    idiomaticSpelling01: allSdsAirtable(filter: { table: { eq: "用字一覧：漢字とひらがな" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticSpelling01: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticSpelling01Query>(query)

  const idiomaticSpelling01 = data.idiomaticSpelling01.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticSpelling01[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticSpelling01.map(({ name, description, discussion, source, recordId }, index) => {
