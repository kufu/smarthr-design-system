...
const query = graphql`
  query IdiomaticNumber {
    idiomaticNumber: allSdsAirtable(filter: { table: { eq: "用字一覧：数字・数え" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticNumber: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticNumberQuery>(query)

  const idiomaticNumber = data.idiomaticNumber.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticNumber[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticNumber.map(({ name, description, discussion, source, recordId }, index) => {
