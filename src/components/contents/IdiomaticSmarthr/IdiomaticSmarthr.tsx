...
const query = graphql`
  query IdiomaticSmarthr {
    idiomaticSmarthr: allSdsAirtable(filter: { table: { eq: "用語一覧：SmartHRの用語" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticSmarthr: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticSmarthrQuery>(query)

  const idiomaticSmarthr = data.idiomaticSmarthr.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticSmarthr[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticSmarthr.map(({ name, description, discussion, source, recordId }, index) => {
