...
const query = graphql`
  query IdiomaticOthers {
    idiomaticOthers: allSdsAirtable(filter: { table: { eq: "用語一覧：その他" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticOthers: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticOthersQuery>(query)

  const idiomaticOthers = data.idiomaticOthers.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticOthers[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticOthers.map(({ name, description, discussion, source, recordId }, index) => {
