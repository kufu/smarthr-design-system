...
const query = graphql`
  query IdiomaticKanaKanji {
    idiomaticKanaKanji: allSdsAirtable(filter: { table: { eq: "用字一覧：漢字とひらがな" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticKanaKanji: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticKanaKanjiQuery>(query)

  const idiomaticKanaKanji = data.idiomaticKanaKanji.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticKanaKanji[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticKanaKanji.map(({ name, description, discussion, source, recordId }, index) => {
