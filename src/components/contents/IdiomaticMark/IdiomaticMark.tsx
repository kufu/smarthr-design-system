...
const query = graphql`
  query IdiomaticMark {
    idiomaticMark: allSdsAirtable(filter: { table: { eq: "用字一覧：記号" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticMark: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticMarkQuery>(query)

  const idiomaticMark = data.idiomaticMark.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticMark[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticMark.map(({ name, description, discussion, source, recordId }, index) => {
