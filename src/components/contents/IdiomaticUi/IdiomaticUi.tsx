...
const query = graphql`
  query IdiomaticUi {
    idiomaticUi: allSdsAirtable(filter: { table: { eq: "用語一覧：コンポーネント" } }) {
      edges {
        node {
          data {
            name
...
export const IdiomaticUi: FC = () => {
  const data = useStaticQuery<Queries.IdiomaticUiQuery>(query)

  const idiomaticUi = data.idiomaticUi.edges
    .map(({ node }) => ({
      name: node.data?.name,
  ...
  return (
    <>
      {idiomaticUi[0].recordId?.includes('MOCKDATA') && (
        <WarningMessage>このページを正しく表示するにはAirtableのAPIキーの設定が必要です</WarningMessage>
      )}
      {idiomaticUi.map(({ name, description, discussion, source, recordId }, index) => {
