...
import { FaTriangleExclamationIcon, Cluster, Text } from 'smarthr-ui'
...
影響する範囲が複雑な場合など特殊な条件が含まれる設定項目には、<FaTriangleExclamationIcon alt="警告" color="WARNING" /> アイコン（`FaTriangleExclamationIcon`）と補足テキストを設定項目の下部に表示しユーザーに注意を促すことで、意図しない選択状態になることを防ぎます。
...
また、操作権限項目の親要素（Panelなど）の幅が狭くなる状況では、設定項目に折り返しが発生します。  
...
コンテンツの量に応じて、操作権限項目の親要素（Panelなど）の幅いっぱいに調整する場合もあります。  
...
（参考: [ラジオボタンの並び順](/products/components/radio-button/#h4-3), [チェックボックスの並び順](/products/components/check-box/#h4-2)）
