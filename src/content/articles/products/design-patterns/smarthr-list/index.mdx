...
- `よくあるリスト`が主に想定しているモバイル環境では表示領域が狭いことから、**ステータスに加え、2行分の情報のみ表示できます**。
...
`よくあるリスト`が主に想定しているモバイル環境では画面の幅が狭いことから、 表示できるボタンは**defaultサイズのアイコンボタン1つです**。複数の操作を表示したい場合は、[DropdownMenuButton](/products/components/dropdown/dropdown-menu-button/)を使います。
...
`FaAngleDownIcon`をprefixに設定した[Tertiaryボタン](/products/components/button/#h3-3)を幅いっぱいに広げて使います。
...
- リストに表示する各項目の独立性が高く、前後の項目と連続した表示を避けたい場合
