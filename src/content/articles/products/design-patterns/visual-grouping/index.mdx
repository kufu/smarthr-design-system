...
余白によるグルーピングでは不都合が生じる際に利用を検討してください。  
...
#### セクションの内容を切り替えるレイアウト
[TabBar](/products/components/tab-bar/)または[SideNav](/products/components/side-nav/)を利用し、セクションの内容を切り替えるコントロールを提供できます。
...
![スクリーンショット:TabBarを利用したセクションの例](./images/section-with-tabbar.png)

##### SideNavを使用する場合
Baseを左右に2分割しSideNavとコンテンツを配置し、左のSideNavで右のコンテンツを切り替えます。

![スクリーンショット:SideNavを利用したセクションの例](./images/section-with-sidenav.png)
...
![スクリーンショット:BaseColumnを利用したブロックの例](./images/block-alt.png)
...
#### TabBarを使用する場合
TabBarを利用する場合は以下のようなレイアウトを採用します。

- TabBarの下に複数セクションを配置したBaseを配置する。
- TabBarによって、Baseを切り替える。

![スクリーンショット:TabBarを利用した複数のセクションを内包するグループの例](./images/sections-tabbar.png)
...
![スクリーンショット:SideNavを利用した複数のセクションを内包するグループの例](./images/sections-sidenav.png)
