...
![スクリーンショット:矩形の例](./images/rectangle.png)
...
<Cluster gap={{ row: 0, column: 1 }}>
  ...
  <DoAndDont type="dont" width="calc(50% - 8px)">
    ...
    <Text slot="label">右カラムの背景色が画面の背景色`BACKGROUND`と同一で境界を認識しづらい。</Text>
  </DoAndDont>
</Cluster>

矩形は[Panel](/products/components/panel/)や[Groupbox](/products/components/groupbox/)を用いて表現できます。
...
### セクション
見出し領域（[セクションタイトル](/products/components/heading/#h3-1)と操作）とコンテンツを内包する[Panel](/products/components/panel/)によって構成されるグループをセクションと呼びます。セクションについての説明はPanelの内部に含めます。[よくあるテーブル](/products/design-patterns/smarthr-table/)もセクションの一種です。

![スクリーンショット:セクションの例](./images/section-basic.png)

基本的に見出し領域とPanelを垂直方向に積み重ねてレイアウトしますが、以下のようにセクションの範囲を視覚的に明確に示したい場合にはPanelの中に見出し領域を含めることもできます。

<ImgWithDesc description="ドラッグ&ドロップによってセクションをユーザーが並べ替える場合">
![スクリーンショット:Panelの中に見出し領域を含める例](./images/section-alt.png)
</ImgWithDesc>
...
![スクリーンショット:TabBarを使用したセクションの例](./images/section-with-tabbar.png)

##### SideNavを使う場合
Panelを左右に2分割しSideNavとコンテンツを配置し、左のSideNavで右のコンテンツを切り替えます。

![スクリーンショット:SideNavを使用したセクションの例](./images/section-with-sidenav.png)
...
コンテンツは基本的に余白でグルーピングしますが、以下のようにまとまりを示しづらい場合には[Groupbox](/products/components/groupbox/)を使い、矩形により明確に領域を示すこともできます。

- RadioButtonやCheckboxが並んでおり、グループの領域がわかりづらい場合
- ドラッグ&ドロップによって項目をユーザーが並べ替える場合
- 前後のブロックのコンテンツがGroupboxでグルーピングされており、視覚的な印象を揃えたい場合（例：下記の画像左）

![スクリーンショット:Groupboxを使用たブロックの例](./images/block-alt.png)
...
- TabBarの下に複数セクションを配置したPanelを配置する。
- TabBarによって、Panelを切り替える。
...
- Panelを左右に2分割しSideNavとコンテンツを配置する。
