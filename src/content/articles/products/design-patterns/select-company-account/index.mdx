...
リストの選択には[RadioButton](/products/components/radio-button/)を使用し、[Panel](/products/components/panel/)で囲むことでオブジェクトを選択していることを強調します。
...
```tsx editable background=DOT canvas=1400 hideCode
<div className="shr-w-full shr-bg-background">
  <Header />
  <Container size="NARROW">
    <Stack gap={1.5}>
      <PageHeading>企業アカウント選択</PageHeading>
      <p>機能名を利用する企業アカウントを選択してください。</p>
      <form>
        <Stack gap={1.5} align="start">
          <Fieldset legend={{ text: "企業アカウント", unrecommendedHide: true }} className="shr-w-full">
            <Stack>
              <RadioButtonPanel name="company" value="company1" label="株式会社スマートエイチアール 1" />
              <RadioButtonPanel name="company" value="company2" label="株式会社スマートエイチアール 2" />
            </Stack>
          </Fieldset>
