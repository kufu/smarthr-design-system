...
| オブジェクトの操作 | ユースケースに応じて表示 | （オブジェクトが詳細ビューを持つ場合）オブジェクトの操作を表示しない<br />（オブジェクトが詳細ビューを持たない場合）defaultサイズのアイコンボタン1つのみ表示 | オブジェクトの操作が複数ある場合は、[DropdownMenuButton](/products/components/dropdown/dropdown-menu-button/)を使ってまとめます。 |
