...
`よくあるテーブル`の実装例です。  
ページレイアウト全体の実装例は[コレクション（テーブル）](/products/design-patterns/page-layout/#h3-7)を参照してください。

```tsx editable hideCode background=BACKGROUND withStyled
const SmartHRTableExample = () => {
  const pagination = {
    currentPage: 1,
    limitValue: 25,
    totalCount: 100,
