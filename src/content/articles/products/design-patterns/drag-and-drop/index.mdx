...
| 用語 | 定義 |
| ---- | ---- |
| インタラクティブな領域 | <Text weight="bold">ユーザーがアイテムをドラッグする操作に反応する領域のこと</Text>です。この領域には、ユーザーがドラッグ操作を開始できる要素を提供する必要があります。 |
| ハンドル | ユーザーがアイテムをドラッグするための特定のインタラクティブな領域のことです。 |
...
- マウスやタッチ入力を使用できないユーザーでも利用できるように、[キーボード操作](#h3-4)も可能にすること
...
<ImgWithDesc description="カード形式では赤い枠の全体をインタラクティブな領域にする">
![カード全体がインタラクティブな領域として示されている様子](./images/smarthr-draggable-list-full-card-handler.png)
</ImgWithDesc>
...
<Cluster gap={1}>
  <DoAndDont type="do" width="calc((100% - 16px) / 2)">
    <Image slot="img" src={doUseHandle} alt="Do" />
    <Text slot="label">カード全体ではなく、ハンドルのみがインタラクティブな領域になっている</Text>
  </DoAndDont>

  <DoAndDont type="dont" width="calc((100% - 16px) / 2)">
    <Image slot="img" src={dontUseCard} alt="Dont" />
    <Text slot="label">カード全体がインタラクティブな領域になっているのに、カードの中にインタラクティブな要素が含まれている</Text>
  </DoAndDont>
</Cluster>
...
推奨するデザインパターンでの実装が難しい場合は、社内Slack`#accessibility_相談`で気軽に相談してください。
...
<Cluster gap={1}>
  <DoAndDont type="do" width="calc((100% - 32px) / 3)">
    <Image slot="img" src={doUseKeyboard} alt="Do" />
    <Text slot="label">ドラッグ&ドロップと同等の機能を、キーボードで操作可能なButtonで提供している。</Text>
  </DoAndDont>

  <DoAndDont type="do" width="calc((100% - 32px) / 3)">
    <Image slot="img" src={doUseKeyboardMenu} alt="Do" />
    <Text slot="label">ドラッグ&ドロップと同等の機能を、キーボードで操作可能なDropdownMenuButtonで提供している。</Text>
  </DoAndDont>

  <DoAndDont type="dont" width="calc((100% - 32px) / 3)">
...
ドラッグ最中のキャンセルができない場合は、ドラッグ完了後に元に戻せるようにしてください。これは、`元に戻す`ボタンやDropdownMenuButtonで提供します。
