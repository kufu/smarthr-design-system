---
title: '一覧画面'
description: '権限を持つアカウントを一覧表示する画面設計をまとめています。'
order: 1
---
...
RBACの独自権限パターンにおいて、権限を持つアカウントを一覧表示する画面の設計をまとめています。

## 画面構成

権限設定の一覧画面では、アカウント一覧のコレクションを表示します。テーブルにはインスタンスの「アカウント名」「ロール」「操作の権限」を表示し、操作列を設けます。
...
- [アカウントの詳細](/products/design-patterns/permissions-settings/rbac-standalone/single/)
