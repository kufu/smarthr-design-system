...
たとえば、F型の視線誘導を意図して見出しの右側にボタンを配置していた場合、画面幅が広くなることで見出しとの距離が大きくなり、ボタンへの視線移動が難しくなる場合があります。

一方、モバイルは画面幅が小さいことから、F型やZ型の視線移動の範囲が小さくなり「視線は上から下に移動する」傾向がより強くなります。

対象とするデバイスの種類や画面幅に合わせてレイアウトを変えるなど、状況に合わせた視線誘導とレイアウト設計を検討してください。
