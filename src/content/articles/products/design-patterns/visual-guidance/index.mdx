...
デジタルプロダクトにおいても、視線誘導の法則を取り入れた画面レイアウトによって、画面の視認性（**パッと見のわかりやすさ**）を高め、以下のようなユーザーが行いたいタスクをより適切に、素早く、数多く処理できる効果を期待できます。
...
モバイルなどの画面幅が小さい状況ではこの傾向がより強くなります。
...
- デバイスや画面幅の違いによって視線誘導の効果が変わることを理解する
...
### デバイスや画面幅の違いによって視線誘導の効果が変わることを理解する
ウェブアプリケーションが表示されるデバイスや画面幅はさまざまです。

ブラウザのウィンドウサイズ変更によって画面や要素の幅が広くなると、F型やZ型の視線誘導が機能しにくくなることがあります。
たとえば、デスクトップでF型の視線誘導を意図して見出しの右側にボタンを配置していた場合、画面幅が広くなることで見出しとの距離が大きくなり、ボタンへの視線移動が難しくなる場合があります。  

一方、モバイルは画面幅が小さいことから視線移動の範囲が小さくなり、「視線は上から下に移動する」傾向がより強くなります。

対象とするデバイスや画面幅に合わせてレイアウトを変えるなど、状況に合わせた視線誘導とレイアウト設計を検討してください。


### 視線誘導のみに頼ったレイアウトをしない（アクセシビリティとの関係）
視線誘導は多数のユーザーには有効ですが、晴眼者を対象とした前提である点に注意してください。

視線誘導の法則を活用したレイアウトだけでなく、[アクセシビリティガイドライン](/accessibility/guidelines/)や、[弱視・ロービジョンのユーザビリティチェックリスト](/accessibility/insight/low-vision/low-vision-check-list/)に対応するなど、目に見える情報だけに頼らないUI設計も検討する必要があります。
...
<Cluster gap={{ row: 0, column: 1 }}>
  ...
  <DoAndDont type="dont" width="calc(50% - 8px)">
    ...
    <Text slot="label">
      ...
      <List>
        <li>見出しがフォーム領域（<a href="/products/components/panel/">Panel</a>）に対して左側に配置されていない</li>
