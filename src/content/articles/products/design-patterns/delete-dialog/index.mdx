...
<>
  <RemoteDialogTrigger targetId="delete-dialog">
    <Button prefix={<FaTrashCanIcon />}>削除</Button>
  </RemoteDialogTrigger>
  <FormDialog
    id="delete-dialog"
    heading="{オブジェクト名}の削除"
  ...
  >
    <Stack gap={1.5}>
      ...
      <DefinitionList>
        <DefinitionListItem term={{ text: '削除する{オブジェクト名}', styleType: 'blockTitle' }}>{`{インスタンス名}`}</DefinitionListItem>
      </DefinitionList>
      <Stack gap={0.5}>
        <BaseColumn>
          <Stack>
            <Heading type="blockTitle" icon={<WarningIcon />}>注意事項</Heading>
            <ul className="shr-ps-1">
        ...
        <BaseColumn>
          <Checkbox name="delete_confirm" required>注意事項を確認しました。</Checkbox>
        </BaseColumn>
      </Stack>
    </Stack>
  </FormDialog>
...
```tsx editable hideCode
<>
  <RemoteDialogTrigger targetId="cancel-dialog">
    <Button>{'{操作名}を取り消す'}</Button>
  </RemoteDialogTrigger>
  <ActionDialog
    id="cancel-dialog"
    heading="{操作名}の取り消し"
    actionButton={{ text: '取り消す', theme: 'danger' }}
