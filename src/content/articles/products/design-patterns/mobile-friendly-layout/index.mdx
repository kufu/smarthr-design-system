...
[メディアクエリのセマンティックトークン](/products/design-tokens/media-query/#h2-0)のうち `SCREEN_NARROW` に一致する環境である場合、その環境はモバイルと判定しモバイルレイアウトの適用対象とします。

<BaseColumn className="shr-mt-2">
  <ResponseMessage>環境がモバイルであるという判定は時代やプロダクトの事情により変わり得ます。簡単に判定できる機能をsmarthr-uiで提供予定です。</ResponseMessage>
</BaseColumn>
