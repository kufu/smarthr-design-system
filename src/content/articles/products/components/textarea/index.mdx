...
Textareaコンポーネントは、[FormControl](/products/components/form-control/)コンポーネントと併用して、ユーザーが何を入力するための項目なのかを示すラベルを設定してください。

これにより、ラベルとTextareaが関連付けられ、スクリーンリーダーを使用するユーザーにも、何を入力するための項目なのかが伝わります。

#### placeholderをラベル代わりに使用しない

`placeholder` は入力例を示すための補助的な機能であり、ラベルの代わりにはなりません。
placeholderは入力中に見えなくなるため、何を入力する項目なのかがわかりにくくなる場合があります。必ずFormControlの`label`でラベルを設定してください。
入力例を示したい場合は、FormControlの`exampleMessage`を利用してください。
