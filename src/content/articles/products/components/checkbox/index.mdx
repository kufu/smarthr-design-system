...
テーブル内にチェックボックスを配置する場合は、専用のコンポーネント（`TdCheckbox`、`ThCheckbox`）を使用してください。
セル内に直接 `Checkbox` コンポーネントを配置すると、チェックボックスと行見出しが関連付かず、スクリーンリーダーでは「どの行に対するチェックボックスか」がわかりにくくなる場合があります。

`TdCheckbox` を利用することで、チェックボックスと同じ行のテキストが関連付けられ、スクリーンリーダーを使用するユーザーにも、どの項目を選択するためのチェックボックスかが伝わりやすくなります。
また、`ThCheckbox` では、「すべての項目を選択 / 解除」のアクセシブルネームが設定されるため、チェックボックスの目的が伝わりやすくなります。
...
<BaseColumn>
  ...
  <BaseColumn bgColor="COLUMN" padding={1.5}>
    {/* 単一のCheckbox  */}
