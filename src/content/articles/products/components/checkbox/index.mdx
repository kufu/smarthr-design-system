...
テーブル内に`Checkbox`を配置する場合は、専用のコンポーネント（`TdCheckbox`、`ThCheckbox`）を使用してください。
セル内に直接`Checkbox`コンポーネントを配置すると、`Checkbox`と行見出しが関連付かず、スクリーンリーダーでは「どの行に対する`Checkbox`か」がわかりにくくなる場合があります。

`TdCheckbox`を利用することで、`Checkbox`と同じ行のテキストが関連付けられ、スクリーンリーダーを使用するユーザーにも、どの項目を選択するための`Checkbox`かが伝わりやすくなります。
また、`ThCheckbox`では、「すべての項目を選択 / 解除」のアクセシブルネームが設定されるため、`Checkbox`の目的が伝わりやすくなります。
...
<Groupbox>
  <Cluster gap={0.5} align="center">
    <Heading type="screenTitle">
      <WarningIcon color="WARNING_YELLOW" />
      注意事項
    </Heading>
  </Cluster>
  
  <Text>
    {オブジェクトの削除に付随する影響について説明する}
  </Text>
  
  <Groupbox bgColor="COLUMN" padding={1.5}>
    {/* 単一のCheckbox  */}
...
// Fieldsetでグループ化されたCheckbox
<Fieldset
  legend="通知設定"
  innerMargin={0.5}
>
  <Cluster gap={{ row: 1, column: 1.5 }}>
    <Checkbox name="emailNotification">メール通知を受け取る</Checkbox>
    <Checkbox name="smsNotification">SMS通知を受け取る</Checkbox>
    <Checkbox name="pushNotification">プッシュ通知を受け取る</Checkbox>
  </Cluster>
</Fieldset>
```
...
```tsx
// ❌ Checkbox とラベルが関連付けられていない
<input type="checkbox" />
<span>利用規約に同意する</span>

// ❌ 何について選択するグループなのか伝わらない
<div>
  <Checkbox name="notification1">メール通知</Checkbox>
  <Checkbox name="notification2">SMS通知</Checkbox>
</div>
```

ラベルが関連付けられていない場合、スクリーンリーダーでは「チェックボックス」とだけ読み上げられ、何を選択するためのチェックボックスかわかりません。
また、グループ化されていない場合、複数のチェックボックスが何についての選択肢なのか伝わりにくくなります。
    ...
    <Container>
      <Stack gap={2}>
        <Stack>
          ...
          <PageHeading autoPageTitle={false}>見出し</PageHeading>
          <p>テキストテキストテキストテキストテキストテキストテキストテキストテキストテキストテキスト</p>
        </Stack>
        <Panel padding={{ block: 1.5, inline: 1}}>
         <Fieldset
            legend="タイトル"
          >
            <Stack>
