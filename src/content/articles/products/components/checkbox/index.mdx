...
#### ラベルとの関連付け
Checkboxコンポーネントは、children（子要素）に渡したテキストが自動的にラベルとして関連付けられます。これにより、スクリーンリーダーを使用するユーザーにもチェックボックスの目的が伝わります。

#### キーボード操作
- <kbd>Tab</kbd>キーでフォーカスの移動が可能
- <kbd>Space</kbd>キーでチェック状態の切り替えが可能
- フォーカス状態が視覚的に明確に表示される

### 開発時の考慮点

#### 必ずラベルを提供する
Checkboxコンポーネントは必ずchildren（子要素）に可視ラベルテキストを提供してください。ラベルがないとスクリーンリーダーを使用するユーザーは、何にチェックを入れているのか理解できません。

#### Fieldsetでグループ化する
複数のCheckboxをグループとして提示する場合は、[Fieldset](/products/components/fieldset/)コンポーネントを使用してグループ化し、legend属性でグループの目的を説明してください。
...
ラベルテキストが関連付けられていない場合、スクリーンリーダーを使用するユーザーには「チェックボックス、チェックなし」のように読み上げられ、何に同意するのかわかりません。また、グループ化されていない場合、複数の選択肢がどのような関係にあるのか理解することが困難になります。
