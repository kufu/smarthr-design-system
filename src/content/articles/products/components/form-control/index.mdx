---
title: 'FormControl'
description: '単一の入力要素にラベルとメッセージテキスト（ヘルプ/エラー/補足）、入力必須かを紐づけるためのコンポーネントです。1つの入力欄にラベル付けするとき、エラーや補足メッセージを表示するときに使います。'
---
...
単一の入力要素にラベルとメッセージテキスト（ヘルプ/エラー/補足）、入力必須かを紐づけるためのコンポーネントです。1つの入力欄にラベル付けするとき、エラーや補足メッセージを表示するときに使います。
...
| エラーメッセージ | `errorMessages` | フォームバリデーションなどで、適切な値が入力されていないことを伝えるテキストを、<FaCircleExclamationIcon alt="エラーメッセージ" color="DANGER" /> アイコン（`FaExclamationCircle`）とあわせて入力要素の上に表示します。 |
