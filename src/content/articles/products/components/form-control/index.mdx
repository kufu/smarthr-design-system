...
Input、Select、Textareaなどの入力要素を使用する場合は、FormControlコンポーネントを利用してラベルを設定してください。
これにより、ラベルと入力要素が関連づけられ、何を入力するための項目なのかが伝わります。

* eslint-plugin-smarthr の `smarthr/a11y-input-in-form-control` ルールは、Input と FormControl の組み合わせを促すものです。有効にして利用することを推奨します。

#### ラベルを設定する

`label` propsには、ユーザーが何を入力するための項目なのかがわかるテキストを設定してください。
...
ラベルと入力要素が関連づけられていない場合、スクリーンリーダーでは何を入力するための項目なのかが伝わりません。
