...
import { ResponseMessage, Groupbox, Cluster, Text, FaCaretDownIcon } from 'smarthr-ui'
...
### 1. トリガー
複数の操作がまとまっていることを示唆するために、[Secondaryボタンのアイコン付き（右）](/products/components/button/#h3-2)に <FaCaretDownIcon alt="展開" /> アイコン（`FaCaretDownIcon`）を使っています。
...
デフォルトではドロップダウンパネル内の操作項目の高さは[`小`サイズのボタン](/products/components/button/#h3-7)に揃えられています。  
モバイルでは、操作しやすいように通常サイズのボタンに揃えてください（TODO: ボタンの基準にリンクする）。

```tsx editable noIframe
<DropdownMenuButton trigger="｛オブジェクト名｝の操作">
  <Button className="shr-py-0.75 shr-border-transparent hover:shr-border-transparent shr-border-solid">｛オブジェクト名｝を編集</Button>
  <Button className="shr-py-0.75 shr-border-transparent hover:shr-border-transparent shr-border-solid">｛オブジェクト名｝を削除</Button>
</DropdownMenuButton>
```
...
<Cluster gap={{ row: 0, column: 1 }}>
  <DoAndDont type="do" width="calc(50% - 8px)">
    ...
    <Text slot="trigger">ドロップダウンパネルの横幅が画面幅の2/3程度に収まっていることで、パネルが閉じやすく、パネルの下のコンテンツとの区別しやすい。</Text>
  </DoAndDont>

  <DoAndDont type="dont" width="calc(50% - 8px)">
    <Image slot="img" src={DropdownMenuButtonPanelWidthDont} alt="Dont" />
    <Text slot="trigger">ドロップダウンパネルの横幅が画面いっぱいに広がっていることで、パネルが閉じづらく、パネルの下のコンテンツと区別しづらい。</Text>
  </DoAndDont>
</Cluster>
...
<Cluster gap={{ row: 0, column: 1 }}>
  <DoAndDont type="do" width="calc(50% - 8px)">
    ...
    <Text slot="trigger">ドロップダウンパネルの縦幅が画面内に収まっていて、パネル内をスクロールすることで必要な操作を押せるようになっている。</Text>
  </DoAndDont>

  <DoAndDont type="dont" width="calc(50% - 8px)">
    <Image slot="img" src={DropdownMenuButtonPanelHeightDont} alt="Dont" />
    <Text slot="trigger">ドロップダウンパネルの縦幅が画面外にはみ出していると、ページをスクロールしたときに誤ってパネル外を押してしまい意図せずパネルが閉じてしまうことがある。</Text>
  </DoAndDont>
</Cluster>
...
| <span style={{ whiteSpace: 'nowrap' }}>ドロップダウンパネル</span> | オブジェクトに対して行う操作を入れます。 |
...
| <span style={{ whiteSpace: 'nowrap' }}>ドロップダウンパネル</span> | オブジェクトに対して行う操作を入れます。トリガーのラベルと同様にオブジェクト名を省けます。 |

```tsx editable noIframe
<DropdownMenuButton trigger={{ size: "S", children: "操作" }}>
  <Button>編集</Button>
  <Button>削除</Button>
</DropdownMenuButton>
```
...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | `trigger.onlyIcon` を設定するとボタンを [アイコンのみ](/products/components/button/#h3-2)で表示できます（標準：`FaEllipsisIcon`）。アイコンの変更は非推奨ですが、やむを得ない場合は`trigger.onlyIcon.component`で変更可能です。なお、何のボタンか識別できるようにするため**必ず`trigger.children`を設定**します。|
| <span style={{ whiteSpace: 'nowrap' }}>ドロップダウンパネル</span> | 操作を複数入れます。 |

```tsx editable noIframe
<Cluster gap={1} align="center">
  <DropdownMenuButton trigger={{ onlyIcon: true, children: "その他の操作" }}>
    <Button>｛操作1｝</Button>
    <Button>｛操作2｝</Button>
    <Button>｛操作3｝</Button>
  </DropdownMenuButton>

  <DropdownMenuButton trigger={{ onlyIcon: true, size: "S", children: "その他の操作" }}>
    <Button>｛操作1｝</Button>
    <Button>｛操作2｝</Button>
    <Button>｛操作3｝</Button>
  </DropdownMenuButton>
...
```tsx editable noIframe
<DropdownMenuButton trigger="｛オブジェクト名｝の操作" disabled>
  <Button>｛オブジェクト名｝を編集</Button>
  <Button>｛オブジェクト名｝を削除</Button>
</DropdownMenuButton>
```
...
#### ドロップダウンパネルの操作の無効
ドロップダウンパネル内の操作を無効にする場合、[ButtonのdisabledReason](/products/components/button/#h3-10)を使って**無効状態の理由を表示します。**
...
<Checklist name="DropdownMenuButton" />

## Props

<ComponentPropsTable name="DropdownMenuButton" showTitle />

<ComponentPropsTable name="DropdownMenuGroup" showTitle />
