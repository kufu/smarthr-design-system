...
| E | トリガーのラベル省略（非推奨） | ボタンを配置するスペースを十分確保できないときの最終手段として検討できますが、基本的に利用を推奨しません。  |
...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | トリガーのラベルは`{オブジェクト名}の操作`とします。 |
...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | 基本的に「A. 単一のオブジェクトに対する操作」のようにオブジェクト名を表示しますが、狭い場所にレイアウトするときにオブジェクト名を省けます。トリガーのラベルは`操作`にします。  |
