...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | [Secondaryボタンのアイコンのみ](/products/components/button/#h3-2)で、主に <FaEllipsisIcon alt="その他の操作" /> アイコン（`FaEllipsisIcon`）を使います。ラベルは視覚的に省いていますが、**必ず`label`属性は設定して**何のボタンなのかを伝えます。 |
