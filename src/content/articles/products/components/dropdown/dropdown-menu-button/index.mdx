...
<Cluster gap={{ row: 0, column: 1 }}>
  <DoAndDont type="do" width="calc(50% - 8px)">
    ...
    <Text slot="label">ドロップダウンパネルの横幅が画面幅の2/3程度に収まっていることで、パネルが閉じやすく、パネルの下のコンテンツとの区別しやすい。</Text>
  </DoAndDont>

  <DoAndDont type="dont" width="calc(50% - 8px)">
    <Image slot="img" src={DropdownMenuButtonPanelWidthDont} alt="Dont" />
    <Text slot="label">ドロップダウンパネルの横幅が画面いっぱいに広がっていることで、パネルが閉じづらく、パネルの下のコンテンツと区別しづらい。</Text>
  </DoAndDont>
</Cluster>
...
| E | トリガー名省略（非推奨） | ボタンを配置するスペースを十分確保できないときの最終手段として検討できますが、基本的に利用を推奨しません。  |
...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | 基本的に「A. 単一のオブジェクトに対する操作」のようにオブジェクト名を表示しますが、狭い場所にレイアウトするときにオブジェクト名を省けます。トリガー名は`操作`にします。  |
| <span style={{ whiteSpace: 'nowrap' }}>ドロップダウンパネル</span> | オブジェクトに対して行なう操作を入れます。トリガー名と同様にオブジェクト名を省けます。 |
...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | [Secondaryボタンのアイコンのみ](/products/components/button/#h3-2)で、主に <FaEllipsisIcon alt="その他の操作" /> アイコン（`FaEllipsisIcon`）を使います。トリガー名は視覚的に省いていますが、**必ず`trigger.children`属性は設定して**何のボタンなのかを伝えます。 |
