...
これを、操作しやすいようにモバイルでは通常サイズのボタンに揃えてください（TODO: ボタンの基準にリンクする）。
...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | [Secondaryボタンのアイコンのみ](/products/components/button/#h3-2)で、主に <FaEllipsisHIcon alt="その他の操作" /> アイコン（`FaEllipsisHIcon`）を使います。ラベルは視覚的に省いていますが、**必ず`label`属性は設定して**何のボタンなのかを伝えます。 |
