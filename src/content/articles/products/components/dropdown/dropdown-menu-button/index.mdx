...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | `trigger.onlyIcon` を設定するとボタンを [アイコンのみ](/products/components/button/#h3-2)で表示できます（標準：`FaEllipsisIcon`）。アイコン変更は`trigger.onlyIcon.component`で可能ですが非推奨です。なお、何のボタンか識別できるようにするため**必ず`trigger.children`を設定**します。|
