---
title: 'SortDropdown'
description: 'コレクションの並べ替えを統一的に提供するドロップダウンコンポーネントです。主にテーブル以外のリストやカード一覧の並べ替え項目と並び順を設定するときに使います。'
---
import ComponentPropsTable from '@/components/article/ComponentPropsTable.astro'
import ComponentStory from '@/components/article/ComponentStory.astro'
import { ResponseMessage, BaseColumn } from 'smarthr-ui'

コレクションの並べ替えを統一的に提供するドロップダウンコンポーネントです。主にテーブル以外のリストやカード一覧の並べ替え項目と並び順を設定するときに使います。
...
const sortAccessary: Record<string, { label: string; icon: React.ReactElement }> = {
  asc: {
    label: '昇順',
    ...
    <>
      ...
      <FormDialog
        ...
        onSubmit={(e, { close }) => {
          ...
          setSortKey(formData.get('sortField') as string)
          setSortOrder(formData.get('sortOrder') as string)
          close()
        }}
      >
        <Stack>
