...
| `小` | <Button prefix={<FaCirclePlusIcon />} size="s">ボタン</Button> | レイアウトの都合上、スペースを節約したいときに使うサイズです。 |
...
| 種類 | 表示例 | アイコンの役割 |
| :--- | :--- | :--- |
| `アイコン付き（左）` | <Cluster><Button prefix={<FaCirclePlusIcon />}>項目を追加</Button><Button prefix={<FaCloudArrowDownIcon />}>ダウンロード</Button></Cluster> | ボタンを押したときに実行される操作を想起させるために使用します。同じ操作のボタンには、同じアイコンを指定してください。 |
| `アイコン付き（右）` | <Cluster><Button suffix={<FaCaretDownIcon />}>一括操作</Button><Button suffix={<FaFilterIcon />}>絞り込み</Button></Cluster> | ドロップダウンのように、ボタンを押したときに特定のUIが表示される場合に使用します。<br />（例：[DropdownMenuButton](/products/components/dropdown/dropdown-menu-button/)、[FilterDropdown](/products/components/dropdown/filter-dropdown/)、[SortDropdown](/products/components/dropdown/sort-dropdown/)） |
