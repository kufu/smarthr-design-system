...
#### モバイルでの操作性に留意する
モバイルでの利用時に仮想キーボードがドロップダウンの表示を狭め、かえって操作性を損なう場合があります。
検索機能が不要な場合は、[Select](/products/components/select/)の使用を検討してください。

詳しくは[選択コンポーネントの使い分け - 検索機能が必要かどうか](/products/design-patterns/selection-component-usage/#h4-4)を参照してください。
