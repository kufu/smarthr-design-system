...
- 必ず[FormControl](/products/components/form-control/)コンポーネントと併用して、Inputに適切なラベルを設定してください。
  - eslint-plugin-smarthrの[smarthr/a11y-input-in-form-control](https://github.com/kufu/eslint-plugin-smarthr/tree/main/rules/a11y-input-in-form-control/)ルールは無効にしないでください。
- 設定したラベルが、Inputの「アクセシブルネーム」として正しく関連付けられていることを確認してください。詳しくは、[フォームパーツにアクセシブルネームがある](/accessibility/check-list/accessible-name/)を参照してください。
...
FormControlを使うと、Inputに明確なラベルが設定され、アクセシブルネームも自動で関連付けられます。
これにより、ユーザーが入力内容を理解しやすくなり、スクリーンリーダーでも正しく読み上げられます。
...
FormControlを使用せず、Input単体でプレースホルダーをラベルの代わりにしています。この方法では、以下の問題があります。

- アクセシブルネームが適切に設定されないため、スクリーンリーダー利用者に入力項目の目的が正しく伝わりません。
- ユーザーが入力を始めるとプレースホルダーが消えてしまい、入力の目的が確認できなくなります。
