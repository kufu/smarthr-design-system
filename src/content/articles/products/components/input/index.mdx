...
## アクセシビリティ
### 基本的なアクセシビリティ機能
- **強制カラーモード対応:** ユーザーがテキストを読みやすくするために[強制カラーモード](https://developer.mozilla.org/ja/docs/Web/CSS/@media/forced-colors)を設定した場合、Inputのボーダーの色が適切に表示されるように実装されています。
- **エラー状態:** [error属性](https://github.com/kufu/smarthr-ui/blob/75eb34543a7bf10e77a251fb110bee7d456d8849/packages/smarthr-ui/src/components/Input/Input.tsx#L163)を設定するとaria-invalid属性が自動的に付与され、スクリーンリーダーでフォーカス時にエラー状態であることが通知されます。
...
- **必ずFormControlと併用する:** Inputコンポーネントは必ずFormControlコンポーネントと併用して、適切なラベルを設定してください。これにより、スクリーンリーダーユーザーが入力項目の目的を理解できるようになります。
  - eslint-plugin-smarthrの[smarthr/a11y-input-in-form-control](https://github.com/kufu/eslint-plugin-smarthr/tree/main/rules/a11y-input-in-form-control/)ルールは、有効のまま使うことをおすすめします。このルールは、InputにFormControlを組み合わせることを促すもので、無効にするとスクリーンリーダーで項目名が読み上げられないリスクがあります。
...
<FormControl 
  title="メールアドレス"
  exampleMessage="例: example@email.com"
>
  <Input 
    name="email" 
    type="email"
...
- プレースホルダーではなくexampleMessageを使用して入力例を示しています
...
- プレースホルダーを入力例として使用していますが、入力開始時に消えてしまうため参照できなくなります。
