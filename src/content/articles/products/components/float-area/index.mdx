...
そこで、モバイルではFloatAreaを画面の下部に固定せず、コンテンツの最後に置くことを推奨します。

![モバイルでFloatAreaをコンテンツの最後に置いた画面](images/float-area-mobile.png)

ただし、FloatAreaを画面の下部に固定したい場合、キーボード入力時に固定を解除すれば問題ありません。
