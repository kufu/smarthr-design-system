...
そこで、FloatAreaを画面の下部に固定せず、対象の領域の後に置くことを推奨します。
...
<Grid>
  ...
  ![ロゴのアイソレーションの領域を超えて要素を配置している画像に、斜線が引かれている](images/float-area-mobile.png)
  </ImgWithDesc>
</Grid>


ただしユースケースに応じて必要な場合はFloatAreaを画面の下部に固定できます。  
仮想キーボードの入力操作の妨げにならないよう、キーボード入力時（入力要素にフォーカス中）はFloatAreaの固定を解除することで、操作領域を確保します。
...
<BaseColumn className="shr-mt-1">
  <ResponseMessage type="info">以下のレイアウトを実現するFloatAreaはsmarthr-uiに実装予定です。</ResponseMessage>
</BaseColumn>
