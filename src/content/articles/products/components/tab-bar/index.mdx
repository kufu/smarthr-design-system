---
title: 'TabBar'
description: '異なるオブジェクトやビューを横方向のタブで切り替えるためのコンポーネントです。同一ページ内で並列関係にあるビューを切り替えるとき、影響範囲を下線で明確にしながらタブを並べるときに使います。'
---
...
異なるオブジェクトやビューを横方向のタブで切り替えるためのコンポーネントです。同一ページ内で並列関係にあるビューを切り替えるとき、影響範囲を下線で明確にしながらタブを並べるときに使います。
...
### IconやBadgeの有無
TabBarItem内に[Icon](/products/components/icon/)や[Badge](/products/components/badge/)を追加することでタブそのものやタブ内のビューに関する情報を補足できます。具体例は以下のとおりです。
...
<TabBar>
  ...
  <TabItem disabled disabledReason={{ message: 'この情報は使えません。' }} id="tabitem3" onClick={() => null}>
    その他
  </TabItem>
</TabBar>
```
...
![タブバーとBaseが並んでいる](./images/tab-bar-stretched.png)
