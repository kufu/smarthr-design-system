...
補足説明や追加情報を提供する場合は`type="description"`を使用し、`Tooltip`の内容を要素のラベルとして利用する場合は`type="label"`を使用してください。

補足説明を提供するのか、アクセシブルネームを提供するのかに応じて適切な値を選択してください。

##### 良い実装例

補足説明を提供する場合は`type="description"`を使用する
...
<Tooltip 
  ...
  type="label"
  triggerType="icon">
  <Button onClick={onClickDownload} size="S">
...
Tooltipの内容をラベルとして利用しているが、`type="description"`（デフォルト）のままになっている

```tsx
// Tooltipの内容がアクセシブルネームとして利用されない
<Tooltip message="追加">
