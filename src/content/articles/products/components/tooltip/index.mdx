...
補足説明テキストをホバーやフォーカスで一時的に表示するためのツールチップコンポーネントです。アイコンのみボタンへのラベル付け、[LineClamp](/products/components/line-clamp/)で省略したテキストの全文表示など、限られたスペースで補足情報を添えるときに使います。
...
- Tooltipによる表示が提供されているコンポーネントにおいて、利用基準に沿った使用
  ...
  - [Button](/products/components/button/) の `disabledReason` props 使用時
...
- [Tooltipのライティング](/products/contents/ui-text/app-writing/#tooltip-writing)
...
アイコンの色には[`TEXT_GREY`](/products/design-tokens/color/#h3-2)を使います。
...
アイコンボタンのラベルなど、Tooltipの内容を要素のアクセシブルネームとして利用する場合は`type="label"`を使用してください。
補足説明を提供する場合は`type`の指定は不要です（デフォルトで補足情報として扱われます）。

`type="label"`を指定すると`aria-labelledby`でトリガー要素に関連付けられ、スクリーンリーダーで要素のラベルとして読み上げられます。
`type`を指定しない場合は`aria-describedby`で関連付けられ、補足情報として扱われます。

##### 良い実装例

アイコンに対する補足説明を提供する（デフォルトなので`type`の指定は不要）
...
Tooltipの内容をラベルとして利用する場合は`type="label"`を使用する
...
```tsx editable noIframe
// ❌ type="label"を指定していないため、Tooltipの内容がアクセシブルネームとして利用されない
<Tooltip message="追加">
  <Button>
    <FaCirclePlusIcon />
  </Button>
...
- [hoverで表示されるコンテンツはポインタを乗せても表示を維持できる | 弱視・ロービジョンのユーザビリティチェックリスト](/accessibility/insight/low-vision/hover/)
...
#### `disabledReason`で無効状態の理由を表示する場合
[無効状態(disabled)のButton](/products/components/button/#h3-10)では、無効状態の理由をTooltipで表示するためのPropsとして `disabledReason` を提供しています。

[よくあるリストのリスト操作エリア](/products/design-patterns/smarthr-list/#h3-1) のように、レイアウト上テキストで理由を表示するスペースを確保できない場合は、無効状態のボタンに対して `disabledReason` を用いてTooltipを表示します。

![よくあるリストの右肩にあるボタンがdisabledになっている。ボタンの上にはdisabledの理由を示すためのツールチップが表示されている](./images/disabled-reason.png)

## 使い方チェックリスト

<Checklist name="Tooltip" />
