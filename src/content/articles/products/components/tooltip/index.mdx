...
#### `disabledDetail`で無効状態の理由を表示する場合
[無効状態(disabled)のButton](/products/components/button/#h3-10)では、無効状態の理由をTooltipで表示するためのPropsとして `disabledDetail` を提供しています。

[よくあるリストのリスト操作エリア](/products/design-patterns/smarthr-list/#h3-1) のように、レイアウト上テキストで理由を表示するスペースを確保できない場合は、無効状態のボタンに対して `disabledDetail` を用いてTooltipを表示します。

![よくあるリストの右肩にあるボタンがdisabledになっている。ボタンの上にはdisabledの理由を示すためのツールチップが表示されている](./images/disabled-detail.png)
