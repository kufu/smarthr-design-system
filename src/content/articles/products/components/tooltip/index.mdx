...
import disabledDetailImage from './images/disabled-detail.png'
...
表示領域が限られている状況で、補足情報の提示が必須になる場合にはやむを得ずTooltipを使用することがあります。

[disabled状態のButton](/products/components/button/#h3-10)では、無効状態の理由を表示するスペースがない場合`disabledDetail`Propsを使用しますが、この場合、理由はTooltipで表示されます。

#### [よくあるリスト](/products/design-patterns/smarthr-list/)の例
<Image src={disabledDetailImage} alt="disabledなButtonの上部にTooltipが表示されているサンプル画像" width={320} />
