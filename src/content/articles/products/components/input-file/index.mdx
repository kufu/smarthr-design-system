...
[FormControl](/products/components/form-control/)と組み合わせた使用する場合、`title`属性でラベルを設定することで、同じ内容がAccessible Nameとしても提供されます。

### InputFileでラベルを提供する
InputFileは「ファイルを選択する」というラベルを提供しているボタンのように見えますが、「ファイルを選択する」だけでは、ユーザーはどのようなファイルを選択すべきかを特定できません。
どんなファイルを選択すべきかを伝えるためのラベルは、[FormControl](/products/components/form-control/)の`title`を利用して別途提供してください。
