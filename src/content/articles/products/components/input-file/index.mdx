...
そのため、必要に応じて`multiple={{ appendable: true }}` propsの利用を検討してください。

`previewable`を有効にしている場合、モバイルではファイルプレビュー（FilePreviewDialog）がフルスクリーン相当のモーダルダイアログで表示されます。デスクトップでは従来どおり、ドラッグやリサイズが可能なモードレスダイアログで表示されます。

### プレビューダイアログ内の検索フォームの非表示
`previewable`を使用している場合、プレビューダイアログ内のPDF検索フォームが画面を圧迫することがあります。`previewable={{ searchable: false }}`を指定することで、プレビューダイアログ内の検索フォームを非表示にできます。

## 使い方チェックリスト

<Checklist name="InputFile" />
