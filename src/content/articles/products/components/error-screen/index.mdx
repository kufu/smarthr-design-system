...
import { AuthErrorScreen, ErrorScreen, ForbiddenErrorScreen, NotFoundErrorScreen, UnauthorizedErrorScreen, UnexpectedErrorScreen } from 'smarthr-ui'
...
`ErrorScreen` はデフォルトで適切な余白が適用されるため、原則として個別にpaddingを指定する必要はありません。
...
```tsx editable
<NotFoundErrorScreen homeUrl="（適切なURLを設定してください）" />
```
...
```tsx editable
<ForbiddenErrorScreen homeUrl="（適切なURLを設定してください）" />
```
...
認証が必要な画面で未認証状態になった場合は、SmartHRへの導線を表示します。

```tsx editable
<AuthErrorScreen smarthrUrl="（適切なURLを設定してください）" />
```
...
```tsx editable
<UnexpectedErrorScreen homeUrl="（適切なURLを設定してください）" />
```
