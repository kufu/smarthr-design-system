...
テーブル内にラジオボタンを配置する場合は、専用のコンポーネント（`TdRadioButton`）を使用してください。
セル内に直接 `RadioButton` コンポーネントを配置すると、ラジオボタンと同じ行のテキストが関連付かず、スクリーンリーダーでは「どの項目を選択するためのラジオボタンか」がわかりにくくなる場合があります。

`TdRadioButton` を利用することで、ラジオボタンと同じ行のテキストが関連付けられ、スクリーンリーダーを使用するユーザーにも、どの項目を選択するためのラジオボタンかが伝わりやすくなります。
...
#### Fieldsetでグループ化する
RadioButtonは、複数の選択肢から1つだけ選択するためのコンポーネントです。
