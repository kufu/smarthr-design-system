...
#### ラベルとの関連付け
RadioButtonコンポーネントは、children（子要素）に渡したテキストが自動的にラベルとして関連付けられます。これにより、スクリーンリーダーを使用するユーザーにもラジオボタンの選択肢が伝わります。

#### キーボード操作
- <kbd>Tab</kbd>キーでラジオボタングループへのフォーカス移動が可能
- <kbd>矢印</kbd>キー（↑↓←→）でグループ内の選択肢間を移動し、自動的に選択される
- フォーカス状態が視覚的に明確に表示される

### 開発時の考慮点

#### 必ずラベルを提供する
RadioButtonコンポーネントは必ずchildren（子要素）に可視ラベルテキストを提供してください。ラベルがないとスクリーンリーダーを使用するユーザーは、何を選択しているのか理解できません。

#### Fieldsetでグループ化する
複数のRadioButtonは、[Fieldset](/products/components/fieldset/)コンポーネントを使用してグループ化し、legend属性でグループの目的を説明してください。これにより、スクリーンリーダーを使用するユーザーは、どのような選択肢のグループなのかを理解できます。

#### 必ず1つの選択肢をデフォルトで選択する
RadioButtonは常に1つの選択肢が選択されている状態にしてください。未選択の状態があると、キーボード操作で選択肢に到達できなくなる場合があります。

#### 良い実装例

```tsx
// Fieldsetでグループ化されたRadioButton
<Fieldset
  legend="通知方法"
  innerMargin={0.5}
>
  <Cluster gap={{ row: 1, column: 1.5 }}>
    <RadioButton name="notification" defaultChecked>
      メール
    </RadioButton>
...
// ❌ デフォルトで選択されている選択肢がない
<Fieldset legend="通知方法">
  <RadioButton name="notification">メール</RadioButton>
  <RadioButton name="notification">SMS</RadioButton>
</Fieldset>
```

ラベルテキストが関連付けられていない場合、スクリーンリーダーを使用するユーザーには「ラジオボタン、選択なし」のように読み上げられ、何を選択するのかわかりません。また、グループ化されていない場合、複数の選択肢がどのような関係にあるのか理解することが困難になります。
