...
チェックボックスは状況に応じて「ON/OFF」といった2択の切り替え入力に使用できます。 

#### 2択の切り替え入力に使用できる場合
説明文やほかの設定項目など前後のコンテキストにより選択肢のコンテキストがわかるラベルになっており、一方の選択肢が明示的になっている場合にチェックボックスを使用できます。

![2択の切り替え入力に使用する際のチェックボックスの使用例](./images/check-box-onoff-ok.png)

利用規約への同意や、[削除ダイアログ](https://smarthr.design/products/design-patterns/delete-dialog/#h3-5)といった重要な操作時に注意事項の確認を促すなど、ユーザーに対して明示的な許諾や確認を求める用途でチェックボックスを使用できます。
...
こうした場合には、選択肢を明示的に表示できる[RadioButton](https://smarthr.design/products/components/radio-button/)を使用してください。
<Cluster gap={{ row: 0, column: 1 }}>
  <DoAndDont type="do" width="calc(50% - 8px)">
  ...
  <DoAndDont type="dont" width="calc(50% - 8px)">
    ...
    <Text slot="label">「従業員に通知しない」という選択肢が暗黙的であり、現在の状態をラベルとチェック状態から推測しなければならない</Text>
  </DoAndDont>
</Cluster>
