...
#### 2択の切り替え入力に使用できる場合
- 選択肢のコンテキストがわかるラベルになっている

説明文やほかの設定項目など前後のコンテキストにより、一方の選択肢が明示的になっている場合はチェックボックスを使用できます。

![2択の切り替え入力に使用する際のチェックボックスの使用例](./images/check-box-onoff-ok.png)

- 利用規約などユーザーに許諾を得る必要がある内容の確認を促す
- [削除ダイアログ](https://smarthr.design/products/design-patterns/delete-dialog/#h3-5)など、重要な操作を行なう場合でユーザーに注意事項の確認を促す
