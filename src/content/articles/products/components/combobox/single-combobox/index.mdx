...
import { Cluster, Text, Groupbox, ResponseMessage } from 'smarthr-ui'
...
値を複数選択させたい場合は[MultiCombobox](/products/components/combobox/multi-combobox/)を使用してください。
...
SingleComboboxは、候補の状態や選択された項目が支援技術でも確認できるよう実装されています。
これにより、候補を確認しながら項目を選択できます。

#### キーボードで操作できる

候補の表示や選択はキーボードで操作できます。
これにより、マウスを利用できないユーザーも候補を検索・選択できます。

### 開発時の考慮点

#### FormControlと併用する

SingleComboboxコンポーネントは必ずFormControlコンポーネントと併用して、ユーザーが何を選択すべきかを示すラベルを設定してください。FormControlと併用することで、支援技術にも選択の目的が伝達されます。

eslint-plugin-smarthrの[smarthr/a11y-input-in-form-control](https://github.com/kufu/eslint-plugin-smarthr/tree/main/rules/a11y-input-in-form-control/)ルールは、SingleComboboxにFormControlを組み合わせることを促すものです。支援技術に選択の目的が伝わらないリスクを防ぐため、有効にして使用することを推奨します。
...
<Groupbox className="shr-mt-2">
  <ResponseMessage>モバイルに適したSingleComboboxを提供予定です。</ResponseMessage>
</Groupbox>

## 使い方チェックリスト

<Checklist name="SingleCombobox" />

## Props

<ComponentPropsTable name="SingleCombobox" />
