...
- 1文目は「何ができる/何の代わりに使うコンポーネントなのか（機能・置換対象）」、2文目は「どのシーンで使うのか（利用シーンの具体例）」を書く。
  - 文末は「〜するときに使います。」または「〜させるときに使います。」で統一する。
- 主語は以下のとおり
  - **入力系（フォーム系の要素）**：ユーザーに値などを入力させる用途のため、システムを主語にして「〜させるとき」基調で記述  
    例:「金額値を入力させるときに使います。」
  - **操作系（上記以外のナビゲーション・操作系の要素）**：ユーザーが能動的に操作する用途のため、ユーザーを主語にして「〜するとき」基調で記述  
    例:「アクションを選択するときに使います。」
