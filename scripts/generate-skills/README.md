...
| 1 (description / deprecated / inheritedBy) | `src/content/articles/products/components/**/index.mdx` の frontmatter | 同リポジトリ内、`parseIndexMdx` で抽出 |
...
- ルータースキル（component-selector）の `description` は「head 文 + 代表シナリオ 5 件（パターン C）」で構成します。代表シナリオは `lib/render-router-skill.ts` 内の `REPRESENTATIVE_SCENARIOS` にハードコードしています（Button / Input / Table / ActionDialog / TextLink）。選定理由・差し替え基準は M4-S8 検証結果（Notion）を参照してください。
- ルータースキルにのみ `paths: ["**/*.tsx", "**/*.jsx"]` を付与します。個別スキルには付与しません（M5 Phase 3 §8 で決定）。
- 派生コンポーネント（`Controlled*` や Table 系子要素など）は親 mdx の `inheritedBy` 宣言で接続し、本文・checklist を継承します。`description` のみ派生先固有の文を採用します。
