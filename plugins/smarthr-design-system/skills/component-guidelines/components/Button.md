...
- [must] アイコンは左右どちらかにのみ指定する
  ...
  - 例外として「操作を想起させる役割」と「特定の UI を表す役割」の両方をアイコンが担う必要があるときは、アイコン付き（右）（サフィックス）に「操作を想起させるアイコン」を指定する場合もある

### レイアウト > アイコンの有無 > アイコン付きボタン > `アイコン付き（左）`（プレフィックス）の判断基準
- [should] SmartHR 上で頻出する操作にはアイコンをつける
  - レイアウトの都合でボタンの存在感を強調したい場合や、隣接するアイコン付きボタンと見た目を揃えたい場合も、必要性があればつけてよい
...
### 状態 > 無効（disabled）
- [avoid] 無効状態のボタンはコントラストが低くユーザーを混乱させるため、できるだけ使わない
...
- [must] 実行されるアクションをボタンラベルで示す

### アクセシビリティ > 開発時の考慮点 > 色以外の情報でもボタンであることを伝える
- [must] Tertiary ボタンや Text ボタンにはアイコンを追加し、色だけでなく形や見た目の違いからも操作できる要素であることが伝わるようにする

### アクセシビリティ > 開発時の考慮点 > 操作内容が伝わるラベルを使用する
- [must] ボタンには、押したときに何が実行されるのかがわかるラベルを設定する
  - ラベルは簡潔でわかりやすく、アイコンや見た目だけに依存せず内容が伝わるようにする

### アクセシビリティ > 開発時の考慮点 > アイコンボタンにはアクセシブルネームを設定する
- [must] アイコンボタンはアイコンコンポーネントの `alt` props に操作内容を表すテキストを設定する

### アクセシビリティ > 開発時の考慮点 > AnchorButtonはリンク先がわかるテキストを使用する
- [must] AnchorButton のテキストはリンク先がわかる文言にする

### モバイル > ボタンの大きさ > 通常サイズのボタンを使う
- [should] モバイルでは基本的に `通常` サイズを使用する
- [avoid] 操作ミスが増えるため、モバイルで `サイズ小` のボタンを使うことは避ける
- [must] レイアウトの都合上 `サイズ小` のボタンにする必要がある場合、間隔を十分にあける
  - その場合、アイコンボタンとして使うことはなるべく避ける

### モバイル > ボタンの大きさ > ボタンの幅
- [should] モバイルでは基本的にボタンをコンテンツ幅いっぱいに広げる
  - 視線誘導やレイアウトを加味して調整が必要な場合は、内包コンテンツに合わせた幅に調整してよい（FloatArea や ActionDialog などでの利用を想定）

### モバイル > 複数のボタンの並べ方 > ボタンを並べる方向
- [should] 複数のボタンは基本的に垂直方向に並べる
  - 視線誘導やレイアウトを加味して調整が必要な場合は、水平方向に並べてもよい
