...
description: "smarthr-ui の StepFormDialog / RemoteDialogTrigger / MessageDialog / FormDialog / ActionDialog を使うとき、props を選ぶとき、関連するアクセシビリティ・デザインシステムのルールを確認するとき、コンポーネントの組み合わせを判断するときに使う。ユーザーに入力や選択などの操作を求めるためのダイアログです。ユーザーは操作内容をシステムに送信できます。"
metadata:
  version: "1.0.0"
...
```ts
import { StepFormDialog, RemoteDialogTrigger, MessageDialog, FormDialog, ActionDialog } from 'smarthr-ui'
```
