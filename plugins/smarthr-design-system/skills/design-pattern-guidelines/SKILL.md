...
## よくある間違い（過去に発生したアンチパターン）

- テーブル操作エリアの追加ボタンに `variant="primary"` を付ける → **付けない**（デフォルトの secondary のまま）
- `Base` の padding を prop で指定する → **誤り**。内側の `Cluster` に `className="shr-p-1"` を使う
- ページネーションを Base 内上部に単独で表示する → **誤り**。Base内上部は PageCounter + withoutNumbers Pagination のセット。フルサイズの Pagination は Base外下部に Center で配置
- `InformationPanel` がない場合に `gap={2}` を使う → **誤り**。`gap={1.5}` を使う
- `StatusLabel` の `type` に `'inactive'` や `'success'` など存在しない値を渡す → **誤り**。有効値は `"error" | "grey" | "blue" | "green" | "red" | "warning"` のみ。必ず `component-guidelines` で確認すること
