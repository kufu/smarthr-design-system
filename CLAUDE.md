...
- `checklist.yaml` — AI エージェント向けのルールチェックリスト（M2 で生成・更新するファイル。YAML 形式。現時点では未配置）。

## checklist.yaml と SKILL.md の関係

`checklist.yaml` は smarthr-component-skills リポジトリ側の SKILL.md 合成スクリプト（`scripts/generate-skills.ts`）の入力になります。
...
`checklist.yaml` は `index.mdx` および `_components/*.mdx` の差分から Claude Code Action が抽出・更新します。M1 で定義された「対象外ルール」の判定はプロンプト側で行うため、このリポジトリにそのリストは持ちません。

## eslint-plugin-smarthr のルール名一覧

`tamatebako` リポジトリの `packages/eslint-plugin-smarthr/rules/` 配下のディレクトリ名一覧を `.github/data/eslint-rule-names.txt` に配置しています。Claude が `checklist.yaml` のルール候補と eslint ルールを対応付ける際の参照情報です。
