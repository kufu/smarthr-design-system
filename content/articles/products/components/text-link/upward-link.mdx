...
### インデント
基本的に、`indent`を有効にしてリンクテキストとメインコンテンツの左端を揃えます。これは「一階層上に移動するリンクである」ことをユーザーに示し、認知を促すための装飾です。

ただし、以下に示すように、<FaArrowLeftIcon alt="左矢印" /> アイコンのインデント分の余白を十分に確保できない場合は例外的に`indent`を無効にします。

#### 例外1. メインコンテンツにサイドナビゲーションが隣接する場合
[基本機能の共通設定](/products/design-patterns-core-features/main-admin/)のように、メインコンテンツにサイドナビゲーションが隣接する場合は、`indent`を無効にします。
