...
![FloatAreaの幅と、画面の下辺からFloatAreaまでの距離の説明](./images/float-area-layout1.png)

#### スクロール固定解除後

- 対象の領域の下辺との間に `1.5（24px）` の余白を取ります。
- FloatAreaの対象ではない領域が続く場合は、関連性が低いことを示せるように、`1.5（24px）` よりも広い余白（最低でも `2（32px）` 、特別な理由がなければ `3（48px）` ）を取ってください。

![FloatAreaの前後の余白の説明](./images/float-area-layout2.png)
