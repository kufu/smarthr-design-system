...
<BaseColumn>
  <WarningIcon text={
    <span>
    LineClampはテキストの全文表示にTooltipが使われることに注意してください。Tooltipはユーザーが能動的に表示しなければならない、拡大表示時に領域外に表示されてしまうなどの課題があるため、LineClampの安易な使用はお勧めしません。基本的にはLineClamp以外のUIの使用を検討してください。
    </span>}
    />
</BaseColumn>
