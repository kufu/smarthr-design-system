...
| 要素 | 説明 |
| :--- | :--- |
| トリガー | [Secondaryボタンのアイコンのみ](/products/components/button/#h3-1)で、主に <FaEllipsisHIcon alt="その他の操作" /> アイコン（`FaEllipsisHIcon`）を使います。ラベルは視覚的に省けますが、**必ず`label`属性は設定して**何のボタンなのかを伝えます。 |
| <span style={{ whiteSpace: 'nowrap' }}>ドロップダウンパネル</span> | 「D. 異なる複数の操作」と同様に、異なる複数の操作を入れます。 |
