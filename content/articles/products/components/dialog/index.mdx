...
複数のモーダルダイアログ（`ActionDialog`や`MessageDialog`）を、1つの操作で複数個表示したり、1つ目のダイアログ上の操作によって2つ目のダイアログを表示したりすることは、以下の理由で原則として推奨されません。3つの画面が重なり、画面どうしの関係性や、どんな操作をすれば何が起こるのかが予測しづらくなるためです。
