...
| アイコンの位置 | 表示例 | アイコンの役割 |
| :-- | :-- | :-- |
| プレフィックス（`prefix`） | <Input prefix={<FaMagnifyingGlassIcon />} /> | 入力内容を想起させるために使用します。 |
| サフィックス（`suffix`） | <Input suffix={<FaCaretDownIcon />} /> | [Combobox](/products/components/combo-box/)のようにフォーカスしたときに特定のUIが表示されるなど、操作を想起させるために使用します。 |
