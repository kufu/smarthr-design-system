...
[HTML 属性: autocomplete](https://developer.mozilla.org/ja/docs/Web/HTML/Attributes/autocomplete)などを参考に適切な`autocomplete`属性を設定してください。

適切な`autocomplete`属性を使用すると、ブラウザが記憶したユーザーの個人情報を自動補完できます。これは言語や記憶に関連した障害を持つユーザーに多大な恩恵のある機能です。またテキストを読むことが困難なユーザーは、`autocomplete`属性に従って支援技術により表示されるアイコンで入力すべき内容を判断できることがあります（参考: [達成基準 1.3.5: 入力目的の特定を理解する](https://waic.jp/translations/WCAG21/Understanding/identify-input-purpose.html)）。
