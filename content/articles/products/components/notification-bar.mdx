...
## 使いどころ
NotificationBarは、ユーザーへ伝えたいフィードバックやメッセージを伝えるために使います。
...
ボタンやリンクを渡せたり、常時表示したい場合は「閉じる」ボタンを消せます。
...
- ユーザーにメッセージが表示されたことを強調するために、`animation={true}`にします。
- ユーザーの明示的な操作で非表示にします。時間の経過で閉じることは非推奨されません(※1)。

#### ダイアログからコレクションにオブジェクトを新規追加した後のメッセージ

以下はコレクションにオブジェクトを追加する際にフィードバックを表示する典型的な例です。  
この例では操作が成功したため、`type="success"`にしていますが、エラーの場合は`error`を指定してメッセージを返します。
...
主に「特別で重要な情報やお知らせを出したいとき」に使う例です。  

- 基本的に、`bold={false}`で使うことを推奨します。破壊的な変更や必ず見てほしい重要情報など必要に応じて強調してください。
- 特別な情報やお知らせで設置するコンポーネントと表示位置は、情報構造に合わせて設計します。代表的な使い方は以下に示します。
