...
| `アイコン付き（右）` | <Cluster><Button suffix={<FaCaretDownIcon />}>一括操作</Button><Button suffix={<FaFilterIcon />}>絞り込み</Button></Cluster> | ドロップダウンのように、ボタン押下後に表示するUIを想起させるために使います。<br />（例：[ドロップダウンボタン](/products/design-patterns/dropdown-button/)、[Dropdown](/products/components/dropdown/)、[FilterDropdown](/products/components/dropdown/filter-dropdown/)） |
