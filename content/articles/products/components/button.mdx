...
また、[ドロップダウンメニュー](/products/components/dropdown/dropdown-button/#h3-1)として使用したり、[アイコンボタン](#h4-1)として使用するなど、ボタンではあるもののボタンの装飾を必要としない場合に使います。
...
| `アイコン付き（右）` | <Cluster><Button suffix={<FaCaretDownIcon />}>一括操作</Button><Button suffix={<FaFilterIcon />}>絞り込み</Button></Cluster> | ドロップダウンのように、ボタン押下後に表示するUIを想起させるために使います。<br />（例：[ドロップダウンボタン](/products/components/dropdown/dropdown-button/)、[Dropdown](/products/components/dropdown/)、[FilterDropdown](/products/components/dropdown/filter-dropdown/)） |
