...
import { Button, Cluster, Tooltip, Text, WarningIcon } from 'smarthr-ui'
...
### Tooltipの使用を避けるケース
Tooltip内の情報は隠れるため、操作に必要な情報の表示への使用は避けましょう。
ユーザーが把握しておかないと操作が進められないような重要な情報は、常に表示しておく方法を検討してください。
