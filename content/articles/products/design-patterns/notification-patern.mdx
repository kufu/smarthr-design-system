...
ユーザーへ伝えたいフィードバックやメッセージをどう表すかを定義しています。
...
アクションへの通知としてユーザーにメッセージを伝えたいケースに使うパターンです。
...
- アクション通知は非推奨となった`FlashMessage`による通知の代替パターンです。
- ユーザーに通知が表示されたことを示唆させやすくするために、`NotificationBar`のanimationプロパティを`true`にします。
- ユーザーへのアテンションを高めるために、`NotificationBar`の`bold`を`true`にすることを推奨しています。
...
以下の動画ではアクション実行後に縦スクロールしても`NotificationBar`が画面上部に表示されるようになっています。

![`NotificationBar`を`positon: stikcy`で追従表示している例](./images/notification-patern/notification-patern-notificationbar-sticky.gif)

### インフォメーション通知

ユーザーへ常に伝えたい、注意のメッセージを伝えたい際に使うパターンです。
主に「特別で重要な情報を出したいとき」などに使います。
...
- インフォメーション通知で設置するコンポーネントの表示位置は、情報構造に合わせて設定します。代表的な配置は次項の[使い方](#h4-1)で述べています。
