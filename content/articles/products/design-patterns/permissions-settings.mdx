...
![1〜3とグルーピングされた赤枠がスクリーンショット上に表示されている。タイトルが1、画面説明テキストが2、テーブルが3でグルーピングされている。](./images/permissions-settings/permissions-settings1.png)

** スクリーンショット：一覧（アカウントと独自の権限設定を扱う場合）**
![1〜3とグルーピングされた赤枠がスクリーンショット上に表示されている。タイトルが1、画面説明テキストが2、テーブルが3でグルーピングされている。3のタイトルは一覧（アカウントのみ）とは異なり、TabBarで表示されている。](./images/permissions-settings/permissions-settings2.png)
...
アカウントのみを扱う場合のセクションタイトルは`アカウント一覧`、オブジェクトの追加ボタンラベルは`追加`としてください。

##### 独自の権限設定がある場合

![スクリーンショット: よくあるテーブルのヘッダー（TabBarを表示するパターン）](./images/permissions-settings/permissions-settings2-heading.png)
...
一覧の[アカウントを追加]、または[(アカウントを)編集]ボタンを押したときに[ActionDialog](/products/components/dialog/#h3-1)で表示します。  
...
![1〜3とグルーピングされた赤枠がスクリーンショット上に表示されている。タイトルが1、アカウントの選択が2、操作の権限が3でグルーピングされている。](./images/permissions-settings/permissions-settings1-dialog.png)

**スクリーンショット: 詳細ダイアログ（独自の権限設定がある場合）**
![1〜4とグルーピングされた赤枠がスクリーンショット上に表示されている。タイトルが1、アカウントの選択が2、操作の権限が3、独自の権限設定が4でグルーピングされている。](./images/permissions-settings/permissions-settings2-dialog.png)
