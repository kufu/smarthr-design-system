...
<Cluster gap={1}>
  <DoAndDont
    type="do"
    img={<StaticImage src={noTitleDo} alt="Do" />}
    label={<Text>画面タイトルとテーブルの間に余白ができない場合は省略できる</Text>}
  />
  <DoAndDont
    type="dont"
    img={<StaticImage src={noTitleDont} alt="Dont" />}
    label={<Text>画面タイトルとテーブルの間にテーブル操作エリアがあり、テーブルの左上に不要な余白ができる</Text>}
    width="auto"
  />
</Cluster>
...
<Cluster gap={1}>
  <DoAndDont
    type="do"
    img={<StaticImage src={onlyPaginationDo} alt="Do" />}
    label={<Text>Base内の上部のエリアごと非表示にする。</Text>}
  />
  <DoAndDont
    type="dont"
    img={<StaticImage src={onlyPaginationDont} alt="Dont" />}
    label={<Text>テーブルのページ送りのみ配置すると、左に不要な余白ができるため、視線誘導として不適切。</Text>}
  />
</Cluster>
...
### TableReelによる列の固定
横スクロールが生じるとき、重要な情報が隠れてしまうことを防ぐため、`TableReel`を用いて列を固定できます。
また、画面の表示幅の変更等により意図せぬ横スクロールが発生した場合にも操作性が担保できます。
