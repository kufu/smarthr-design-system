---
title: '権限設定の画面'
description: 'SmartHRにおける「権限設定」画面のパターンをまとめています。'
order: 8
---

SmartHRにおける「権限設定」画面のパターンをまとめています。

## 基本的な考え方

SmartHRのような業務アプリケーションにおいて、権限設定はなくてはならない重要な機能です。  
基本機能を除く権限設定の画面パターンには以下のバリエーションがあります。
...
![スクリーンショット: 一覧ビュー（アカウントのみ）](./images/admin-screen/admin-screen-admin1.png)

![スクリーンショット: 一覧ビュー（アカウント以外の独自の権限設定がある）](./images/admin-screen/admin-screen-admin2.png)
...
一覧ビューのよくあるテーブルの[アカウントを追加]、または[(アカウントを)編集]ボタンをクリックすると表示されるダイアログを想定しています。
...
![スクリーンショット: 詳細ダイアログ（アカウントのみ）](./images/admin-screen/admin-screen-admin1-dialog.png)

![スクリーンショット: 詳細ダイアログ（アカウント以外の独自の権限設定がある）](./images/admin-screen/admin-screen-admin2-dialog.png)
...
アカウントに対して操作の権限（役割）設定します。
プロダクトによって詳細は異なりますが、操作の権限は**「機能管理者」「業務担当者」**の2つの役割を設定します。
...
操作の権限をユーザーが任意に増やせる**「カスタム権限」**を作れるプロダクトもあります。
このケースでは[SingleComboBox](/products/components/combo-box/)、または[Select](/products/components/select/)の利用を検討してください。

#### B-4.  独自のオブジェクト設定

各プロダクトの独自の権限オブジェクト設定が必要な場合に表示します。
機能ごとに設定が異なるため、現状は特にルールや決まったパターンを設けません。
