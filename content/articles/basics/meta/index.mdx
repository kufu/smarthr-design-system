...
ウェブサイトの基本情報であるメタ情報についてまとめています。そのなかで下記のメタ情報をまとめた実装例を紹介します。それぞれの詳細についてリンク先のページを参照してください。

- [ファビコン・アイコン](/basics/meta/favicon)
- [OGP](/basics/meta/ogp)
- [テーマカラー](/basics/meta/theme-color)

1. 下記のコードを`<head>`タグ内に記述する

属性の値は実際の状況に合わせて適宜変更してください。
...
2. 後述するファビコンやアイコンのファイルをサイトルートに配置する

この実装例では、各ファイルはサイトルートに配置することが前提になっていますが、実際の実装では施策に合わせて適宜ファイル名やパスを変更してください。`_readme.txt`は内容を確認し、実際に配置する際には事前に削除してください。

`manifest.webmanifest`にもアイコンやタイトルの情報が含まれています。これも実際の状況に合わせて適宜変更してください。
